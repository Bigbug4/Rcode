...
under <- ovun.sample(class ~ ., data, method = "under", N=50,seed = 12)$data
table(under$class)
...
for(i in 1:k){
  ...
  summary(mymodel)
  svm.pred <- predict(mymodel, testset[,-102])
  svm.pred_p <- predict(mymodel, testset[,-102],probability=T)
  svm.pred.p <- as.numeric(attr(svm.pred_p,"probabilities")[,1])
  # n <- ifelse(svm.pred>0,1,0) 
  # n <- ifelse(svm.pred==z,1,0)
  # sum(n)
  tab <- table(svm.pred,testset[,102])
  res <- as.numeric(tab)
  ...
  svmtest_y <- as.character(testset[,102])
