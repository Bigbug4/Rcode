...
write.csv(expr_data_choosed,file = "expr_gene_data.csv")
...
heatmap(log10(as.matrix(expr_data_choosed)))
...
write.csv(expr_all,"expr_all_stomach.csv")
# scale
library(edgeR)
...
write.csv(expr_all_log,"expr_all_log.csv",row.names = FALSE)
...
write.csv(roc_plot,file ="roc_test.csv",row.names = FALSE)

#数据???
expr_gene <- read.csv("expr_gene_data.csv", row.names = 1)
...
write.csv(expr_gene,file = "expr_gene_choosed.csv")
expr_gene <- read.csv("expr_gene_choosed.csv",row.names = 1)
expr_gene_log <- log10(subset(expr_gene, select = -class))
expr_gene_log$class <- expr_gene$class
write.csv(expr_gene_log,file = "expr_gene_log_choosed.csv",row.names = FALSE)

sample_all_test <- read.csv("Sample_all_test.csv")
expr_gene_all <- read.csv("expr_all_stomach.csv", row.names = 1)
...
write.csv(expr_gene_all,file = "expr_gene_all.csv")
